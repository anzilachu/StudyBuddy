...
def loginPage(request):
    ...
    if request.user.is_authenticated:
            return redirect('home') #already login ?, then no need to go to login page again
...
def home(request):
    ...
    room = Room.objects.filter(Q(topic__name__icontains=q) | Q(description__icontains=q) | Q(name__icontains=q) )
...
def update(request,pk):
    room = Room.objects.get(id=pk)
    form = RoomForm(instance=room)#prefilled the already data in form by instance the room value
    topics = Topic.objects.all()
...
def delete(request,pk):
    room = Room.objects.get(id=pk)
    if request.user != room.host:
        return HttpResponse('you are not allowed here !!') #an another user may delete someone else's room
